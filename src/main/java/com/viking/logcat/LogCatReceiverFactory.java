...
/**
 * A factory for {@link com.android.ddmuilib.logcat.LogCatReceiver} objects. Its primary objective is to cache
 * constructed {@link com.android.ddmuilib.logcat.LogCatReceiver}'s per device and hand them back when requested.
 */
public class LogCatReceiverFactory {
    /** Singleton instance. */
    public static final LogCatReceiverFactory INSTANCE = new LogCatReceiverFactory();

    private Map<String, com.android.ddmuilib.logcat.LogCatReceiver> mReceiverCache = new HashMap<String, com.android.ddmuilib.logcat.LogCatReceiver>();
    ...
    private synchronized void removeReceiverFor(IDevice device) {
        com.android.ddmuilib.logcat.LogCatReceiver r = mReceiverCache.get(device.getSerialNumber());
        if (r != null) {
            r.stop();
    ...
    public synchronized com.android.ddmuilib.logcat.LogCatReceiver newReceiver(IDevice device, IPreferenceStore prefs) {
        com.android.ddmuilib.logcat.LogCatReceiver r = mReceiverCache.get(device.getSerialNumber());
        if (r != null) {
            return r;
        }

        r = new com.android.ddmuilib.logcat.LogCatReceiver(device, prefs);
